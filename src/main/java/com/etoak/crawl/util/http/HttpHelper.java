...
import com.etoak.crawl.util.enums.Encodings;
import com.etoak.crawl.util.enums.HttpMethod;
import com.etoak.crawl.util.enums.StatusCode;
import com.etoak.crawl.util.exceptions.InnerException;
import com.etoak.crawl.util.text.RegexUtil;
import com.etoak.crawl.util.text.StringUtil;
import com.etoak.crawl.util.utils.HtmlPages;
//import com.jxl.cloud.model.bean.HtmlPages;
//import com.jxl.cloud.model.enums.ContentTypes;
//import com.jxl.cloud.model.enums.Encodings;
//import com.jxl.cloud.model.enums.HttpMethod;
//import com.jxl.cloud.model.enums.StatusCode;
//import com.sheng.cloud.util.exceptions.InnerException;
//import com.sheng.cloud.util.text.RegexUtil;
//import com.sheng.cloud.util.text.StringUtil;
//import com.sheng.cloud.utils.utils.StringUtils;
import com.etoak.crawl.util.utils.StringUtils;
...
import static com.etoak.crawl.util.consts.Consts.TIMEOUT;
...
public abstract class HttpHelper {
    ...
    public String goGetRetry(String url, Integer retryTime, String token) {
        return goGetRetry(url, retryTime, token, Encodings.UTF8);
    }

    public String goGetRetry(String url, Integer retryTime, String token, Encodings encodings) {
        String result = "";
        for (int i = 0; i < retryTime; i++) {
            LOGGER.info("[{}] [{}] goGetRetry [{}] ...", token, url, i);
            result = goGet(url, encodings);
            if (StringUtils.isNotBlank(result) && !"timeout".equals(result)) {
                break;
            }
        }
        return result;
    ...
    //********************PUT***********************
    public String goPut(String url) {
        return goPut(url, Encodings.UTF8);
    }

    public String goPutString(String url) {
        updateCookieString();
        return goPut(url, Encodings.UTF8);
    }

    public String goPut(String url, Encodings responseEncoding) {
        byte[] bytes = goPutBytes(url);
        if (StringUtil.isEmpty(bytes)) {
            return TIMEOUT;
        }
        return new String(bytes, Charset.forName(responseEncoding.getValue()));
     ...
     * @return
     */
    public byte[] goPutBytes(String url) {
        try {
            HttpResponse httpResponse = goPutResponse(url);
            if (StringUtil.isEmpty(httpResponse)) {
                return null;
            }
            return EntityUtils.toByteArray(httpResponse.getEntity());
    ...
    public HttpResponse goPutResponse(String url) {
        return request(url, HttpMethod.PUT, null);
    }
    ...
    public HttpResponse goPostResponse(String url, String params, Encodings requestEncoding) {
        ...
        switch (contentType) {
            ...
            case FORM:
                ...
                break;
            case STRING:
                httpEntity = new StringEntity(params,  Charset.forName(requestEncoding.getValue()));
                break;
            default:
    ...
    public HttpResponse request(String url, HttpMethod method, HttpEntity entity) {
        ...
        try {
            ...
            if (!StringUtil.isEmpty(requestBase)) {
                ...
                requestBase.setHeader(userAgentName, userAgent);
                ...
                enclosingRequestBase.setHeader(userAgentName, userAgent);
